...
function InputPhone({
  ...
  return (
    <div className={boxClassName}>
      <p className={`text-[16px] mb-2 font-Medium ms-1 ${labelClassName}`}>
        {label}
      </p>
      <div dir="ltr" className="mx-1">
        <PhoneInput
          ...
          autoFormat={false}
          inputClass={`!rounded-lg h-11 w-full text-base border-0 bg-[#D9D9D980] ps-[70px] placeholder:text-base ${inputClassName}`}
          inputProps={{
            type: "text",
            required: true,
            className: ` py-2 w-full text-base border-0 bg-[#D9D9D980] h-11  !rounded-lg ps-[70px] outline-none ${inputClassName}`,
            placeholder: "",
            id: "phone",
          }}
          buttonClass={`  border ${
          ...
          }   w-[64px] flex !rounded-s-lg items-center justify-center !top-1/2 transform -translate-y-1/2 !bg-white`} // For flag dropdown
