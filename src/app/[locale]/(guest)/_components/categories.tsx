    ...
    <div className="mb-10 lg:mb-16">
      ...
      <div className="flex justify-center flex-wrap mt-7 lg:mt-10 gap-1 lg:gap-7 ">
        {CategoriesData.map((category, index) => {
          return (
            <Link
              href={ROUTES.GUEST.PRODUCTS(category.title)}
              key={category.id}
              className="pt-3 px-2 mdl:px-3 pb-1 rounded-xl w-[90px] mdl:w-[108px] h-[102px] min-h-max flex flex-col justify-between  border border-transparent items-center gap-3 duration-300 hover:shadow-sidebar hover:border hover:border-green"
            >
              {category.img}
              <h3 className="text-sm lg:text-[16px] font-SemiBold text-center flex-1 place-content-center">
                {category.title}
              </h3>
            </Link>
          );
